...
func (s *DockerCLIRunSuite) TestRunWithUlimits(c *testing.T) {
	...
	out := cli.DockerCmd(c, "run", "--name=testulimits", "--ulimit", "nofile=128", "busybox", "/bin/sh", "-c", "ulimit -n").Combined()
	ul := strings.TrimSpace(out)
	if ul != "128" {
		c.Fatalf("expected `ulimit -n` to be 128, got %s", ul)
	}
}
...
func (s *DockerDaemonSuite) TestRunWithUlimitAndDaemonDefault(c *testing.T) {
	...
	name = "test-B"
	_, err = d.Cmd("run", "--name", name, "--ulimit=nofile=128", "-d", "busybox", "top")
	assert.NilError(c, err)
	assert.NilError(c, d.WaitRun(name))

	out, err = d.Cmd("inspect", "--format", "{{.HostConfig.Ulimits}}", name)
	assert.NilError(c, err)
	assert.Assert(c, strings.Contains(out, "[nofile=128:128]"))
}
