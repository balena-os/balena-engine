...
func TestDeltaCreate(t *testing.T) {
	...
	pullBaseAndTargetImages(ctx, t, client, base, target)
	...
	go func() {
		...
		for br := bufio.NewReader(rc); ; {
			...
			if err != nil {
				...
				t.Errorf("read delta progress stream: %v", err)
				return
			}
...
func pullBaseAndTargetImages(ctx context.Context, t *testing.T, client apiclient.APIClient, base, target string) {
	var (
		err error
		rc  io.ReadCloser
	)
