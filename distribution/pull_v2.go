...
func (ld *layerDescriptor) Read(p []byte) (int, error) {
	if ld.downloadRetries > 0 {
		...
		for sleepDurationInSecs > 0 {
			if ld.ctx.Err() == context.Canceled {
				...
				log.G(ld.ctx).Info("context canceled during wait, interrupting layer download")
	...
	case nil:
		if ld.downloadRetries > 0 {
			log.G(ld.ctx).Infof("download resumed after %v retries", ld.downloadRetries)
	...
	case context.Canceled:
		...
		log.G(ld.ctx).Info("context canceled, interrupting layer download")
	default:
...
func (p *puller) pullSchema2Layers(ctx context.Context, target distribution.Descriptor, layers []distribution.Descriptor, platform *ocispec.Platform) (id digest.Digest, err error) {
	...
	// Pull the image config synchronously before starting layer downloads.
	// We need the config before starting the layer download to prepare for
	// a delta pull if this config happens to be a delta config.
...
func logDownloadRetry(retries int, format string, args ...interface{}) {
	...
	case retries < logEveryRetryLimit:
		log.G(context.TODO()).Infof(format, args...)
	case retries == logEveryRetryLimit:
		log.G(context.TODO()).Infof(format+" (and reducing log frequency from now on)", args...)
	case (retries-logEveryRetryLimit)%reducedLogInterval == 0:
		log.G(context.TODO()).Infof(format+" (logging with reduced frequency)", args...)
	}
}
