...
func installCommonConfigFlags(conf *config.Config, flags *pflag.FlagSet) error {
	...
	flags.StringVar(&conf.DeltaRoot, "delta-data-root", conf.DeltaRoot, "Root directory of read-only balenaEngine state used for deltas")
	...
	flags.IntVar(&conf.MaxUploadAttempts, "max-upload-attempts", config.DefaultUploadAttempts, "Set the max upload attempts for each push")
	...
	flags.StringVarP(&conf.Root, "graph", "g", conf.Root, "Root of the balenaEngine runtime")
