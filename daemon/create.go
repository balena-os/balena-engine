...
func setContainerIDEnv(container *container.Container, cidenv string) error {
	...
	log.G(context.TODO()).WithField("balenaext", "cidenv").Debugf("adding %s=%s to container environment", cidenv, container.ID)
	return nil
}
...
func (daemon *Daemon) DeltaCreate(deltaSrc, deltaDest string, options types.ImageDeltaOptions, outStream io.Writer) error {
	return daemon.imageService.DeltaCreate(deltaSrc, deltaDest, options, outStream)
}
