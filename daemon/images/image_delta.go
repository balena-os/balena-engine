...
func (i *ImageService) DeltaCreate(deltaSrc, deltaDest string, options types.ImageDeltaOptions, outStream io.Writer) error {
	...
	srcImg, err := i.GetImage(ctx, deltaSrc, imagetypes.GetImageOpts{})
	if err != nil {
		return errors.Wrapf(err, "no such image: %s", deltaSrc)
	}

	dstImg, err := i.GetImage(ctx, deltaDest, imagetypes.GetImageOpts{})
	if err != nil {
	...
	for idx, diffID := range dstImg.RootFS.DiffIDs {
		...
		dstRootFS.DiffIDs = dstRootFS.DiffIDs[:idx+1]

		if idx < len(srcImg.RootFS.DiffIDs) {
			srcRootFS := *srcImg.RootFS
			srcRootFS.DiffIDs = srcRootFS.DiffIDs[:idx+1]
		...
		if commonLayer {
			...
			layerProgressReader := progress.NewProgressReader(input, progressOutput, inputSize, stringid.TruncateID(diffID.String()), "Computing delta")
			defer layerProgressReader.Close()
			...
			go func() {
				...
				err := librsync.Delta(srcSig, bufio.NewReader(layerProgressReader), w)
				if err != nil {
	...
	if err := i.TagImage(ctx, id, ref); err != nil {
		return err
	}
	log.G(ctx).Debugf("Tagged delta %s with %s", id.String(), reference.FamiliarString(ref))
