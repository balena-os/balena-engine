...
func (i *ImageService) DeltaCreate(deltaSrc, deltaDest string, options types.ImageDeltaOptions, outStream io.Writer) error {
	...
	srcImg, err := i.GetImage(ctx, deltaSrc, backend.GetImageOpts{})
	if err != nil {
		return errors.Wrapf(err, "no such image: %s", deltaSrc)
	}

	dstImg, err := i.GetImage(ctx, deltaDest, backend.GetImageOpts{})
	if err != nil {
...
func newImageLock(ls layer.Store, img *image.Image) (*imglock, error) {
	var lock imglock
	for idx := range img.RootFS.DiffIDs {
		rootFS := *img.RootFS
		rootFS.DiffIDs = rootFS.DiffIDs[:idx+1]
